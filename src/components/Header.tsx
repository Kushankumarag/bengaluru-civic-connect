    ...
    <header className="sticky top-0 z-50 glass-nav">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-4">
          ...
          >
            {languages.map(lang => (
              <option key={lang} value={lang} className="bg-civic-dark">{lang}</option>
            ))}
          </select>
          ...
          <Link to="/admin-signup">
            <Button className="bg-civic-accent text-civic-dark hover:bg-opacity-80 glow-hover transition-all duration-300 bg-slate-900 hover:bg-slate-800 text-teal-400">
              Admin Sign Up
            </Button>
          </Link>
        </nav>
